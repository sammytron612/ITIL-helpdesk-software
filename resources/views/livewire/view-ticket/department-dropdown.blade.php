<div class="flex">
    <div x-transition x-data="{open: false}" @click="open = ! open" @click.outside="open = false">
        <div class="relative flex items-center font-bold cursor-pointer">
            ...
            <div x-cloak class="absolute right-0 z-30 rounded-md shadow-lg bg-slate-300 top-7 focus:outline-none"
                ...
                <div class="py-1">
                    @foreach($departments as $department)
                        <div class="hover:bg-slate-200">
                            <button wire:click="updateDepartment('{{$department->id}}')" class="block px-8 py-2 text-sm text-gray-700 hover:text-blue-900">{{$department->name}}
                            </button>
                        </div>
                    @endforeach
                </div>
