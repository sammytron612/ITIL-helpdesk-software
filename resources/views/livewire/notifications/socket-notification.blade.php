<div x-on:click.outside="dropDown = false" class="relative z-50" x-data="{dropDown: false}">
    <span x-on:click="dropDown = ! dropDown" class="relative inline-block hover:cursor-pointer">
        ...
        @if($count)<span class="absolute top-0 right-0 flex items-center justify-center w-4 h-4 p-1 text-xs font-bold text-white bg-red-600 rounded-full">{{$count}}</span>@endif
    </span>
...
lklkll
         @foreach($notifications as $notification)   
            <div>
                <button wire:click="" class="block w-full px-4 py-2 text-sm text-left text-gray-700 hover:bg-gray-200 hover:text-blue-900">{{$notification['message']}} hjghj jgjhjghjghjghjgj hjg jg hgjgjg</button>
            </div>
        @endforeach
        </div>
    </div>
