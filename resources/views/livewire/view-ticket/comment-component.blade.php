        ...
        <div class="mt-5">
            <div x-transition.duration.500ms x-show="updates">
                @foreach($comments as $comment)
                    <div wire:key="{{$comment->id}}" x-data="{ openComment: false, newEditor: false, kbOpen: false }"  class="p-5 border-2 shadow rounded-t-xl">
                        ...
                        <div  x-on:click.outside="kbOpen = false" class="relative">
                            ...
                            <button x-show="newEditor" x-on:click="updateComment({{$comment->id}});updates = true; comment = false; newEditor = false; destroyEditor({{$comment->id}})" class="mt-2 btn-primary">Update</button>
                            <button x-show="newEditor" x-on:click="updates = true; comment = false; newEditor = false; destroyEditor({{$comment->id}})" class="mt-2 btn-secondary">close</button>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div wire:ignore x-transition.duration.500ms x-show="comment">
                ...
                <button x-on:click="new_comment();updates = true; comment = false" class="px-4 py-2 mt-3 text-white bg-blue-500 rounded hover:bg-blue-400">Post</button>
            </div>
...
/*
    ClassicEditor
        .create( document.querySelector( '#new' ),{
            link: {
            addTargetToExternalLinks: true
        },
                extraPlugins: [ MyCustomUploadAdapterPlugin ]
