<div class="px-5 lg:px-0" x-data="category">
    <div class="w-full mx-auto text-left lg:w-2/3">
        ...
        <input class="ml-2" {{$locationActive ? 'checked' : ''}} wire:model="locationActive" type="checkbox" />
    </div>
    <article class="w-full mx-auto mt-3 text-left lg:w-2/3">Map each location to a ticket group. When tickets are created they will be automatically assigned to that group depending on location</article>
    <div class="w-full mx-auto mt-5 overflow-y-auto h-96 lg:w-2/3">
        <table class="mx-auto">
            <thead>
                <th class="w-4 text-left border-b border-gray-500"></th>
                <th class="text-lg text-left border-b border-gray-500 w-96">Location</th>
            ...
            <tbody>
                @foreach($locations as $location)
                    <tr class="pt-2" x-data="{openSelect: false}" x-on:click.outside="openSelect = false">
                        <td></td>
                        <td class="pt-3"><button class="font-semibold hover:text-blue-700 hover:font-bold" x-on:click="openSelect = true; categoryChoice({{$location->id}})" class="text-lg font-bold">{{$location->name}}</button></td>
                        <td class="pt-3" x-data="{choice: null}">
                            @if(isset($rules))
                                @foreach($rules as $rule)
                                    @if($rule->location == $location->id)
    ...
    <script>
        function category(){
            return {
                ...
                completeCheck(){
                    if(this.location && this.group)
                    {
                        @this.addRule(this.location,this.group)
                    }
                    return
